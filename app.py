...
# ─── Native Streamlit sign-out button ─────────────────────────────────────────
# Placed right below the navbar, right-aligned. Native st.button → Python fires
# directly. No iframe, no JS, no query params — cannot silently fail.
st.markdown("""
<style>
  div.rv-signout-wrap {
    display: flex;
  ...
  div.rv-signout-wrap + div div[data-testid="column"]:last-child .stButton > button {
    background: transparent !important;
  ...
  div.rv-signout-wrap + div div[data-testid="column"]:last-child .stButton > button:hover {
    background: #FEF2F2 !important;
    border-color: #B91C1C !important;
    color: #991B1B !important;
  }
  div.rv-signout-wrap + div div[data-testid="column"]:last-child .stButton > button:focus {
    box-shadow: none !important;
    outline: none !important;
  }
  div.rv-signout-wrap + div div[data-testid="column"]:last-child {
    display: flex;
    justify-content: flex-end;
  }
</style>
<div class="rv-signout-wrap"></div>
""", unsafe_allow_html=True)

_so_left, _so_right = st.columns([10, 1])
with _so_right:
    if st.button("Sign out", key="rv_signout_btn"):
        logout()
